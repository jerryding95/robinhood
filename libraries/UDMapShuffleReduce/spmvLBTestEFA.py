...
class SPMVMapShuffleReduce(UDKeyValueMapShuffleReduceTemplate):
    ...
    def kv_map(self, tran: Transition, key: str, values: list, map_ret_ev_label: str) -> int:
        ...
        tran.writeAction(f"sri {key} {inter_key} {32}")
        tran.writeAction(f"movir {value} 10")
        tran.writeAction(f"fcnvt.i64.64 {value} {value}")
        # tran.writeAction(f"print 'multiply %llu by 10 as %llu' {values[0]} {value}")
        tran.writeAction(f"fmul.64 {values[0]} {value} {value}")
        # tran.writeAction(f"print 'get result %llu' {value}")
        '''
        ...
        '''
        self.kv_emit(tran, inter_key, [value], dest_id, self.scratch[0], self.scratch[1])
...
def spmvLBTestEFA():
    ...
    '''
    testMSR = SPMVMapShuffleReduce(task_name="test", num_workers=64*1, partition_parm = 1, enable_cache = True, lm_mode = 1, debug_flag = False, extension = SPMVMapShuffleReduce.AvailableExtensions.lb_test)
    ...
    # init_tran.writeAction("movir UDPR_5 1")
    # init_tran.writeAction(f"addi X7 UDPR_1 0")
    # init_tran.writeAction(f"movrl UDPR_5 {TERM_FLAG_ADDR}(UDPR_1) 0 8")
    # init_tran.writeAction("yieldt")
    ...
    # init_tran.writeAction(f"evii UDPR_9 {testMSR.get_event_mapping('updown_terminate')} 255 5")
    # init_tran.writeAction(f"evi UDPR_9 UDPR_9 6 {0b0010}")
    # init_tran.writeAction(f"addi X2 UDPR_10 0")
    # init_tran.writeAction(f"evlb UDPR_10 {testMSR.get_event_mapping('updown_terminate')}")
    # init_tran.writeAction(f"sendops_wcont UDPR_9 UDPR_10 OB_0 6")
    '''
    ...
    # init_tran.writeAction(f"sendr_wret UDPR_9 {testMSR.get_event_mapping('updown_terminate')} OB_0 OB_1 UDPR_10")
    init_tran.writeAction("movir UDPR_0 0")
    init_tran.writeAction(f"addi X7 UDPR_1 0")
    init_tran.writeAction(f"movrl UDPR_0 {TERM_FLAG_ADDR}(UDPR_1) 0 8")
    init_tran.writeAction(f"print 'Init event yield'")
...
#     efa.add_state(state)
#     tran1 = state.writeTransition("eventCarry", state, state, 1)

#     # init_tran.writeAction(f"evii UDPR_9 {testMSR.get_event_mapping('updown_terminate')} 255 5")
#     tran1.writeAction(f"evii X16 0 255 5")
#     tran1.writeAction(f"sendr_wret X16 0 X17 X18 X19")
#     tran1.writeAction(f"yieldt")

#     tran0 = state.writeTransition("eventCarry", state, state, 0)
#     tran0.writeAction("movir UDPR_5 1")
#     tran0.writeAction(f"addi X7 UDPR_1 0")
#     tran0.writeAction(f"movrl UDPR_5 {TERM_FLAG_ADDR}(UDPR_1) 0 8")
#     tran0.writeAction("yieldt")
#     tran0.writeAction(f"yieldt")
