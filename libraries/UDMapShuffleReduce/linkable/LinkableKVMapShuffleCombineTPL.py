...
class UDKeyValueMapShuffleReduceTemplate(metaclass=ABCMeta):
    ...
    def __init__(self, efa: EFAProgram, task_name: str, meta_data_offset:int, debug_flag: bool = False, 
                    ...
                    claim_multiple_work = True, do_all_reduce = False, grlb_type = 'ud', test_map_ws=False, test_reduce_ws=False, random_lb=False):
        '''
        Parameters
        ...
        self.global_reduce_work_stealing = test_reduce_ws
        ...
        self.any_work_stealing = self.intra_map_work_stealing or self.inter_map_work_stealing or self.global_map_work_stealing \
                                or self.intra_reduce_work_stealing or self.inter_reduce_work_stealing or self.global_reduce_work_stealing or random_lb
        ...
        if self.extension == 'load_balancer' and self.launching_worker:
            ...
            if 'reducer' in self.lb_type:
                ...
                self.intermediate_cache_entry_size      = 4 if self.grlb_type=='lane' else 5
        ...
        if 'reducer' in self.lb_type:
            ...
            if self.grlb_type == 'ud': 
                ...
                if self.claim_multiple_work:
                    ...
                    if self.do_all_reduce:
                        ...
                        tran.writeAction(f"movrl {scratch[1]} {WORD_SIZE*3}({addr}) 0 {WORD_SIZE}")
                        ...
                        # if self.debug_flag:
                        #     tran.writeAction(f"movlr {WORD_SIZE * 2}({addr}) {materialized_count} 0 {8}")
                        #     tran.writeAction(f"print 'Num entry reads 0x%lx' {materialized_count}")
                        #     tran.writeAction(f"movlr {WORD_SIZE * 2}({addr}) {materialized_count} 0 {4}")
                        #     tran.writeAction(f"print 'Num entry first 4 bytes reads 0x%lx' {materialized_count}")
                        #     tran.writeAction(f"movlr {WORD_SIZE * 2 + 4}({addr}) {materialized_count} 0 {4}")
                        #     tran.writeAction(f"print 'Num entry last 4 bytes reads 0x%lx' {materialized_count}")
                        #     tran.writeAction(f"movlr {WORD_SIZE * 2 + 4}({addr}) {materialized_count} 0 {8}")
                        #     tran.writeAction(f"print 'Num entry last 4 bytes and next 4 bytes reads 0x%lx' {materialized_count}")
                        # tran.writeAction(f"movlr {WORD_SIZE * 2 + 4}({addr}) {materialized_count} 0 {4}")
                        tran.writeAction(f"movlr {WORD_SIZE * 2}({addr}) {materialized_count} 0 {4}")
                        tran.writeAction(f"beqi {materialized_count} {0} claim_multiple_work_set_materialized_count")
                        tran.writeAction(f"bnei {dest} {-1} claim_multiple_work_set_materialized_count")
                        tran.writeAction(f"movlr {WORD_SIZE}({addr}) {dest} 0 {WORD_SIZE}")
                        tran.writeAction(f"movlr {WORD_SIZE * 2}({addr}) {materialized_count} 0 {4}")
                        ...
                        if self.debug_flag:
                            tran.writeAction(f"print 'Local claimed key %lu with dest %lu, count %lu, entry address %lu' {tmp} {dest} {materialized_count} {addr}")
            ...
            if self.do_all_reduce and self.do_all_reduce_with_materialize:
                if self.grlb_type == 'lane':
                    ...
                    # Send dram request to read intermediate kv
                    tran.writeAction(f"dram_iteration_claim: beqi {claiming_count} 0 claiming_finished")
                    tran.writeAction(f"send_dmlm_ld_wret {scratch[1]} {self.ln_worker_launch_reducer_ev_label} {self.inter_kvpair_size} {scratch[0]}")
                    if self.debug_flag:
    ...
    def __gen_worker_claim_remote_work(self, tran):
        ...
        if not self.any_work_stealing:
            tran.writeAction(f"movir {scratch[0]} {1}")
            tran.writeAction(f"sli {scratch[0]} {scratch[0]} {27+32}")
            tran.writeAction(f"or {scratch[0]} {ev_word} {ev_word}")
        if self.claim_multiple_work:
    ...
    def __gen_worker_claimed_reduce_task(self, tran):
        ...
        if self.do_all_reduce:
            ...
            if self.grlb_type == 'ud':
                ...
                # tran.writeAction(f"movir {scratch[1]} {5}")
                # tran.writeAction(f"sli {scratch[1]} {scratch[1]} {27+32}")
    ...
    def __gen_worker_confirm_local_materialized_count(self, tran):
        ...
        num = "UDPR_1"
        addr = "UDPR_7"                             # UDPR_7                            local reg
        dest = "UDPR_8" if self.grlb_type == 'ud' else 'X9'
        ...
        if self.grlb_type == 'ud':
            tran.writeAction(f"sli {num} {num} {32}")
            tran.writeAction(f"sri {num} {num} {32}")
            tran.writeAction(f"movlr {WORD_SIZE}({'X8'}) {dest} 0 {WORD_SIZE}")
        if self.debug_flag:
            tran.writeAction(f"movlr 0({'X8'}) {scratch[0]} 0 {WORD_SIZE}")
            tran.writeAction(f"sri {scratch[0]} {scratch[0]} 1")
            tran.writeAction(f"print 'Lane %u confirm local materialized count of key %u as %ld, previously %ld, entry address %lu, dram ptr %lu' X0 {scratch[0]} {num} {'X10'} X8 {dest}")
        ...
        tran.writeAction(f"add {dest} {scratch[1]} {scratch[1]}")
    ...
    def __gen_worker_launch_reducer(self, tran):
        ...
        if self.enable_intermediate and not self.enable_cache:
            ...
            # tran.writeAction(f"sendops_wcont X2 X1 {'X8'} {self.inter_kvpair_size + 1}")
            tran.writeAction(f"yield")
    ...
    2 cache space:
        intermediate_cache: Each entry contains FOUR words, [*key, ptr to materize array / event word at claimed lane, num_dram_req_sent, num_dram_req_ret]
                            *key here is key shifted left one bit, with lowest bit as is_claimed bit
                            [*key, -1/ptr to materize array, num kv materialized, num_dram_req_sent, num_dram_req_ret]
    ...
    def __gen_receiver_receive_kv(self, tran):
        ...
        if self.grlb_type == 'ud':
            ...
            if self.debug_flag:
                tran.writeAction(f"print 'Ptr next reads %ld.' {scratch[0]}")
            ...
            if self.debug_flag:
                tran.writeAction(f"print 'Entry %ld reads %ld, not hit' {addr} {scratch[0]}")
        ...
        tran.writeAction(f"movlr {WORD_SIZE*3}({entry_addr}) {ev_word} 0 {WORD_SIZE}")
        ...
        if self.grlb_type == 'ud':
            ...
            tran.writeAction(f"{check_kv_ptr_label}: movlr {WORD_SIZE}({entry_addr}) {ev_word} 0 {WORD_SIZE}")
            if self.debug_flag:
                tran.writeAction(f"print 'Lane %u key %ld materialized address %lx' X0 X8 {ev_word}")
            tran.writeAction(f"blti {ev_word} {0} {init_cache_kv_label}")
        ...
        if self.grlb_type == 'ud':
            ...
            tran.writeAction(f"update_materialize_count: movlr 0({entry_addr}) {scratch[0]} 0 {WORD_SIZE}")
            ...
            tran.writeAction(f"jmp update_materialize_count")

            # Calculate dram address
            tran.writeAction(f"updated_materialize_count: sli {num} {num} {32}")
            tran.writeAction(f"sri {num} {num} {32}")
        ...
        if self.grlb_type == 'ud':
            ...
            # Double check if new entry has been inserted before grabbing the lock
            tran.writeAction(f"movlr {(self.intermediate_cache_entry_size - 1) * WORD_SIZE}({addr}) {scratch[0]} 0 {WORD_SIZE}")
            if self.debug_flag:
                tran.writeAction(f"print 'Double check Ptr next reads %ld.' {scratch[0]}")
            tran.writeAction(f"beqi {scratch[0]} -1 double_checked")
            tran.writeAction(f"movir {scratch[0]} {0}")
            tran.writeAction(f"movrl {scratch[0]} 0({bin_lock_addr}) 0 {WORD_SIZE}")
            tran.writeAction(f"jmp {iterate_key_label}")


            tran.writeAction(f"double_checked: movir {scratch[1]} {self.inter_key_received_count_offset}")
            tran.writeAction(f"add {base_addr} {scratch[1]} {scratch[1]}")
            ...
            if self.debug_flag:
                tran.writeAction(f"sri X0 {scratch[1]} 6")
                tran.writeAction(f"print '[LB_DEBUG] Lane %u receives new key %ld as ud %lu %lu-th key, entry %ld' X0 X8 {scratch[1]} {num} {entry_addr}")
            ...
            # 3.2 Set number materialized as 0 indicating materializing address hasn't returned. LOCK FOR RECEIVE_KEY
            tran.writeAction(f"movir {scratch[1]} {0}")
            tran.writeAction(f"movrl {scratch[1]} {WORD_SIZE*2}({entry_addr}) 0 {WORD_SIZE}")
            # 3.3 Set entry pointer word as -1, indicating this entry as end of the bin. FOR HASHTABLE
            tran.writeAction(f"movrl {scratch[0]} {WORD_SIZE*(self.intermediate_cache_entry_size-1)}({entry_addr}) 0 {WORD_SIZE}")
            ...
            tran.writeAction(f"movrl {scratch[0]} 0({bin_lock_addr}) 0 {WORD_SIZE}")
            if self.debug_flag:
                tran.writeAction(f"print 'Lane %d released bin lock for new key %ld' X0 X8")
            ...
            # tran.writeAction(f"movrl {scratch[0]} {WORD_SIZE*1}({scratch[1]}) 0 {WORD_SIZE}")
            # tran.writeAction(f"movrl {scratch[0]} {WORD_SIZE*(self.intermediate_cache_entry_size-1)}({scratch[1]}) 0 {WORD_SIZE}")
        ...
        if self.grlb_type == 'ud':
            ...
            # Decrement num by 1
            tran.writeAction(f"movir {scratch[1]} {1}")
            tran.writeAction(f"sli {scratch[1]} {scratch[1]} {32}")
            tran.writeAction(f"update_num: movlr {WORD_SIZE*2}({entry_addr}) {num} 0 {WORD_SIZE}")
            if self.debug_flag:
                tran.writeAction(f"print 'materialized count %lx' {num}")
            ...
            tran.writeAction(f"add {num} {scratch[1]} {scratch[0]}")
            tran.writeAction(f"cswp {addr} {cswp_ret} {num} {scratch[0]}")
            tran.writeAction(f"bne {num} {cswp_ret} update_num")
            tran.writeAction(f"sri {num} {num} {32}")
            ...
            # #   If this is the first cache entry, write cache end offset to cache_offset
            # tran.writeAction(f"bgei {num} {0} write_cache_kv")
            # tran.writeAction(f"movrl {WORD_SIZE + 5}({entry_addr}) {materializing_cache_end} 0 {3}")

            tran.writeAction(f"write_cache_kv: movrl X8 0({addr}) 1 {WORD_SIZE}")
    ...
    def __gen_receiver_fetched_kv_ptr_for_cache(self, tran):
        if self.grlb_type == 'lane':
            ...
            if self.debug_flag:
                tranreceiver__fetched_kv_ptr_for_cache.writeAction(f"print 'Receive_kv_ptr decrement unresolved_kv_count from %ld to %ld' X18 X19")
            ...
            if self.debug_flag:
                tranreceiver__fetched_kv_ptr_for_cache.writeAction(f"print 'Receive_kv_ptr dram ptr %lu key %ld claimed %ld' X8 X19 X20")
            ...
            if self.debug_flag:
                tranreceiver__fetched_kv_ptr_for_cache.writeAction(f"__if_fetched_kv_ptr_for_cache_39_true: print 'Materialize to dram %ld' X22") 
                tranreceiver__fetched_kv_ptr_for_cache.writeAction(f"send_dmlm X22 X26 X23 {self.inter_kvpair_size}") 
            else:
        ...
        if 'reducer' in self.lb_type:
            ...
            if self.grlb_type == 'ud':
                ...
                if self.do_all_reduce and self.do_all_reduce_with_materialize:
                    ...
                    # Send dram request to read intermediate kv
                    tran.writeAction(f"dram_iteration_claim: beqi {claiming_count} 0 receiver_claim_work_end")
                ...
                if self.claim_multiple_work:
                    ...
                    if self.do_all_reduce:
                        ...
                        if self.debug_flag:
                            tran.writeAction(f"movlr 0({addr}) {part_end} 0 {WORD_SIZE}")
                            tran.writeAction(f"sri {'X1'} {scratch[0]} {32}")
                            tran.writeAction(f"print 'Remote claimed key %lu by lane %d, %ld-th at %lu' {part_end} {scratch[0]} {entry_ind} {addr}")
                        ...
                        # If num>0 && dest==-1, needs to re-read dest
                        tran.writeAction(f"beqi {num} {0} write_dest_to_send_buffer")
                        tran.writeAction(f"bnei {dest} {-1} write_dest_to_send_buffer")
                        tran.writeAction(f"movlr {WORD_SIZE}({addr}) {dest} 0 {WORD_SIZE}")
                        tran.writeAction(f"movlr {WORD_SIZE * 2}({addr}) {num} 0 {WORD_SIZE}")
                        tran.writeAction(f"write_dest_to_send_buffer: movrl {dest} {WORD_SIZE}({scratch[1]}) 0 {WORD_SIZE}")
                        ...
                        # Update claimed event word
                        tran.writeAction(f"movrl {cswp_ret} {WORD_SIZE*3}({addr}) 0 {WORD_SIZE}")
                        tran.writeAction(f"movrl {scratch[0]} 0({addr}) 0 {WORD_SIZE}")

                        if self.debug_flag:
                            tran.writeAction(f"sri {scratch[0]} {part_end} {1}")
                            tran.writeAction(f"sri {'X1'} {scratch[0]} {32}")
                            tran.writeAction(f"print 'Remote claimed key %lu by lane %d, %ld-th with dram address %lu(0x%lx) and number %d at %lu ' {part_end} {scratch[0]} {dest} {num} {entry_ind} {addr}")
    ...
    def __gen_receiver_check_materialized_count(self, tran):
        ...
        # tran.writeAction(f"movir {self.scratch[1]} {0}")
        if self.grlb_type == 'ud':
            tran.writeAction(f"sli {self.scratch[1]} {self.scratch[1]} {32}")
            tran.writeAction(f"sri {self.scratch[1]} {self.scratch[1]} {32}")
        ...
        tran.writeAction(f"movlr {WORD_SIZE}({'X8'}) {dest} 0 {WORD_SIZE}")
        tran.writeAction(f"add {dest} {self.scratch[0]} {self.scratch[0]}")
