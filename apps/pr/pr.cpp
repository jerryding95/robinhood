#include "PagerankMsrEFA_nlb.hpp"
#include "PagerankMsrEFA_ws.hpp"
#include "PagerankMsrEFA_rh_nlbstrm_off.hpp"
#include "PagerankMsrEFA_rh_nlbstrm_on.hpp"
#include "PagerankMsrEFA_rh_random.hpp"
// #include "simupdown.h"
...
int main(int argc, char* argv[]) {
  ...
  std::unordered_map<std::string, int> event_labels = {
    {"nlb", PagerankMsrEFA_nlb::updown_init},
    {"ws", PagerankMsrEFA_ws::updown_init},
    {"rh_nlbstrm_off", PagerankMsrEFA_rh_nlbstrm_off::updown_init},
    {"rh_nlbstrm_on", PagerankMsrEFA_rh_nlbstrm_on::updown_init},
    {"rh_random", PagerankMsrEFA_rh_random::updown_init}
  };
  ...
  UpDown::BASimUDRuntime_t* pagerank_rt = new UpDown::BASimUDRuntime_t(machine, "PagerankMsrEFA_"+mode+".bin", 0, 1);
  ...
  uint64_t* interSpace;
  if (mode == "ws") {
    interSpace = gen_intermediate_kv(pagerank_rt, num_nodes * num_uds_per_node * 64, 512, 512*2);
  }
  else if(mode != "nlb") {
    interSpace = gen_intermediate_kv_ud(pagerank_rt, num_nodes * num_uds_per_node, 512*64, 512*2);
  }
