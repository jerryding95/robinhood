...
output_dir="$UPDOWN_DATA_DIR/pr"
reorder_tool="$UPDOWN_SOURCE_CODE/install/updown/apps/preprocess_pagerank"
...
for i in "${!graphs[@]}"; do
    ...
    if [[ -f "$input_file" ]]; then
        ...
        "$reorder_tool" "$input_file"
        mv "${input_file}.bin" "$output_dir/"
        mv "${input_file}_gv.bin" "$output_dir/"
        mv "${input_file}_nl.bin" "$output_dir/"
    else
