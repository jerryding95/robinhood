...
from KeyValueSetTPL import KeyValueSetTemplate, Transition
...
class PagerankMapShuffleReduce(UDKeyValueMapShuffleReduceTemplate):
    ...
    def kv_reduce_loc(self, tran: Transition, key: str, nwid_mask: str, dest_id: str):
        tran.writeAction(f"rshift {key} {dest_id} {6}")
        tran.writeAction(f"and {dest_id} {nwid_mask} {dest_id}")
        return 
...
def GeneratePageRankMapShuffleReduceEFA() -> EFA:
    ...
    '''
    pagerankMSR = PagerankMapShuffleReduce(task_name="pr", meta_data_offset=64, debug_flag=False)
    ...
    pagerankMSR.set_max_thread_per_lane(max_map_th_per_lane=16, max_reduce_th_per_lane=110)
