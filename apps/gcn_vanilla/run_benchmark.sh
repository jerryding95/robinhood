...
graph_dir="$UPDOWN_DATA_DIR/data"
...
mode=$1
num_nodes=$2
perflog_path="gcn_udkvmsr_exe_${mode}.bin.logs/perflog.tsv"
save_perflog_path="~/gnn_perflog"
...
for i in "${!graphs[@]}"; do
	...
	cat $perflog_path
	# echo mv $perflog_path ${save_perflog_path}/${graphs[$i]}_${1}_perflog.tsv
	# mv $perflog_path ${save_perflog_path}/${graphs[$i]}_${1}_perflog.tsv
	# cp -r ${graphs[$i]}_${1}_perflog ${save_perflog_path}/
done
