...
'''
TASK_NAME       = "DistributedSortPhase1"
METADATA_OFFSET = Defs.UDKVMSR_0_OFFSET
DEBUG_FLAG      = False
DEBUG_FLAG_CACHE      = False

MAX_MAP_THREAD_PER_LANE     = 25
MAX_REDUCE_THREAD_PER_LANE  = 80
...
INPUT_KVSET = OneDimKeyValueSet("Dsort_input", element_size=1, metadata_size=4)
# INTERMEDIATE_KVSET = OneDimKeyValueSet("Dsort_intermediate", element_size=2, metadata_size=4)
INTERMEDIATE_KVSET =IntermediateKeyValueSet("Dsort_intermediate", key_size=1, value_size=1)
...
def GenKMerCntMSREFA(efa: EFAProgram):
    ...
    cache = LMCache(state=testMSR.state, identifier="phase1_bin_size_cache",
     cache_offset = SORT_OFFSET + COUNTERS_OFFSET,
    # cache_offset = SORT_OFFSET + COUNTERS_OFFSET + 2000 * 8, 
    num_entries = 64, policy=LMCache.Policy.WRITE_BACK, 
            entry_size = 2, data_store = cache_data_store, metadata_offset = Defs.SHT_0_OFFSET, debug_flag = DEBUG_FLAG_CACHE, combine_func=cache_combine_func)
