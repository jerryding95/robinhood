...
#include "spmvMSR_nlb.hpp"
#include "spmvMSR_ws.hpp"
#include "spmvMSR_rh_nlbstrm_off.hpp"
#include "spmvMSR_rh_nlbstrm_on.hpp"
#include "spmvMSR_rh_random.hpp"
...
void test_lb(UpDown::UDRuntime_t* rt, std::string mode, uint64_t* args, int arglen){
    std::unordered_map<std::string, int> event_labels = {
        {"nlb", spmvMSR_nlb::updown_init},
        {"ws", spmvMSR_ws::updown_init},
        {"rh_nlbstrm_off", spmvMSR_rh_nlbstrm_off::updown_init},
        {"rh_nlbstrm_on", spmvMSR_rh_nlbstrm_on::updown_init},
        {"rh_random", spmvMSR_rh_random::updown_init}
    };
    std::cout << "Executing mode: " << mode << std::endl << ", Event label: " << event_labels[mode] << std::endl;
    ...
    UpDown::event_t evnt_ops(event_labels[mode],                     /*Event Label*/
                            nwid,
...
int main(int argc, char* argv[]) {
    ...
    uint64_t num_nodes = strtol(argv[2], nullptr, 0);
    std::string fpath = argv[3];
    int mtx_mode = std::stoi(argv[4]);

    if(mode == "rh") {
        mode = "rh_nlbstrm_on";
    }
    uint64_t num_uds = num_nodes * 32;
    ...
    UpDown::UDRuntime_t* rt = initialize_rt("spmvMSR_"+mode, num_nodes, num_uds_per_node, num_lanes_per_ud);
    ...
    uint64_t* interSpace;
    if (mode == "ws") {
        interSpace = gen_intermediate_kv(rt, num_workers, 256, 256*2);
    }
    else if(mode != "nlb") {
        interSpace = gen_intermediate_kv_ud(rt, num_uds, 256*64, 256*2);
    }
